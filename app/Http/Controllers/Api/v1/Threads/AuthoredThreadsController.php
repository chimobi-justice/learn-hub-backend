...
class AuthoredThreadsController extends Controller
{
    ...
    /**
     * @OA\Get(
     *      path="/threads/authored/{username}/public",
     *      tags={"threads"},
     *      summary="Fetch paginated threads by an authored using a username,  please pass the authored username",
     *      description="Returns paginated threads by an authored using a username, if limit is provided 10 by default",
     *      security={{"bearer_token": {}}},
     *      @OA\Parameter(
     *          name="limit",
     *          in="query",
     ...
     *  )
     */
    public function getPublicAuthoredThreads(Request $request, $username) {
        try {
            $user = User::where('username', $username)->firstOrFail();
